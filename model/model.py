...
class AdaptiveInstanceNorm(jt.Module):
...
class EqualLR:
    ...
    def compute_weight(self, module):
        ...
        # fan_in = weight.data.size(1) * weight.data[0][0].numel()#diff
        fan_in = weight[0].numel()

        return weight * jt.sqrt(2 / fan_in)
...
class FusedUpsample(jt.Module):
    def __init__(self, in_channel, out_channel, kernel_size, padding=0):
        ...
        self.multiplier = jt.sqrt(2 / fan_in)
        self.weight = weight
        self.bias = bias
...
class ConstantInput(jt.Module):
    def __init__(self, channel, size=4):
        ...
        self.input = jt.randn(1, channel, size, size)#diff
...
class Discriminator(jt.Module):
    ...
    def execute(self, input, step=0, alpha=-1):
        for i in range(step, -1, -1):
            ...
            if i == 0:
                # out_std = jt.sqrt(out.var(0, unbiased=False) + 1e-8)
                # mean_std = out_std.mean()#diff
